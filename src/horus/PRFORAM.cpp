...
void PRFORAM::setupInsert(map<Bid, string> values, map<Bid, int> poses) {
    vector<Box*> nodes;
    auto valuesIterator = values.begin();
    auto posesIterator = poses.begin();
    viewmap.clear();
    ...
    sort(nodes.begin(), nodes.end(), [ ](const Box* lhs, const Box * rhs) {
    ...
    while (i < nodes.size()) {
        ...
        for (int d = depth; d >= 0 && i < nodes.size() && curPos == nodes[i]->pos; d--) {
            int nodeIndex = GetBoxOnPath(curPos, d);
            Bucket bucket;
            if (bucketsCnt.count(nodeIndex) == 0) {
                ...
                for (int z = 0; z < Z; z++) {
                    if (i < nodes.size() && nodes[i]->pos == curPos) {
                        ...
                        curBlock.data = convertBoxToBlock(nodes[i]);
                        delete nodes[i];
